#!/bin/bash
# External smoke check for https://eye-eye.ropca.kz (run from anywhere).
set -euo pipefail

BASE="${1:-https://eye-eye.ropca.kz}"

echo -n "login page: "
LOGIN_CODE=$(curl -s -o /tmp/eyeeye-login.html -w "%{http_code}" "${BASE}/login")
echo "$LOGIN_CODE"
...
CSS_PATH=$(grep -oE '/_next/static/chunks/[^"]+\.css' /tmp/eyeeye-login.html | head -1)
if [ -z "$CSS_PATH" ]; then
