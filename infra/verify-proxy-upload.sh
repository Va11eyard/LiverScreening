...
CODE=$(curl -s -b "$COOKIE" -o /tmp/upload-resp.json -w "%{http_code}" -X POST "$WEB/api/proxy/cases/${CASE_ID}/images" \
  -F "images=@${PNG};type=image/png")

if [ "$CODE" != "200" ] && [ "$CODE" != "201" ]; then
  echo "FAIL: POST /api/proxy/cases/${CASE_ID}/images returned HTTP $CODE" >&2
  cat /tmp/upload-resp.json >&2
  exit 1
fi

echo "OK: image upload works (HTTP $CODE) for case $CASE_ID"
